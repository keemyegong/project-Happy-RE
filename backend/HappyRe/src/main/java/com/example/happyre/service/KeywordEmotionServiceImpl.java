...
public class KeywordEmotionServiceImpl implements KeywordEmotionService{
    ...
    public KeywordEmotionEntity insertDTO(KeywordEmotionDTO keywordEmotionDTO){
        ...
        if(!findByKeywordAndEmotionString(keywordEntity, KeywordEmotionEntity.getEmotionEntity().getEmotion()).isEmpty()) throw new RuntimeException("동일한 Keyword에 동일한 emotion 이 매핑된 keywordEmotion이 이미 존재함.");
