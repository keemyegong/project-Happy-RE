...
wss.on('connection', (ws) => {
  ...
  users[userId] = { id: userId, ws: ws, position: { x: Math.random() * 2 - 1, y: Math.random() * 2 - 1 }, pipeline: null, webRtcEndpoint: null };
  ...
  ws.on('message', async (message) => {
    ...
    switch (data.type) {
      ...
      case 'offer':
        ...
        break;
      case 'candidate':
        handleCandidate(userId, data.candidate);
        break;
    }
  });
...
function handleMove(userId, position) {
  ...
  if (nearbyUsers.length > 0) {
    connectUsers(userId, nearbyUsers[0]);
  }
}
